...
mysql -u root -plibo13630760268 -e "DELETE FROM mysql.user WHERE User='';"
mysql -u root -plibo13630760268 -e "DELETE FROM mysql.user WHERE User='root' AND Host NOT IN ('localhost', '127.0.0.1', '::1');"
mysql -u root -plibo13630760268 -e "DROP DATABASE IF EXISTS test;"
mysql -u root -plibo13630760268 -e "DELETE FROM mysql.db WHERE Db='test' OR Db='test\\_%';"
# 设置密码并切换成密码验证
mysql -u root -plibo13630760268 -e "ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY 'libo13630760268';"
